...
func (t *PatientChaincode) invoke(stub shim.ChaincodeStubInterface, args []string) pb.Response {
...
func (t *PatientChaincode) queryone(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	patAsBytes, _ := stub.GetState(args[1])
...
func (s *PatientChaincode) createPat(stub shim.ChaincodeStubInterface, args []string) pb.Response {
...
func (s *PatientChaincode) gethistory(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	if len(args) < 2 {
		return shim.Error("Incorrect number of arguments. Expecting 3")
	}
...
func main() {
	...
	if err != nil {
		fmt.Printf("Error starting Patient chaincode: %s", err)
	}
}
